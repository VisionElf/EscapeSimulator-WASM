...
impl Walk {
    ...
    pub fn find_field_offset(
        ...
        let initial_class = class.address;
        let mut this_class = Some(class);

        loop {
            let class = this_class?;

            // An explicitly requested Unity class can expose its own fields.
            // Keep the original stop rule when climbing from a game class.
            if class.address != initial_class && (self
